...
- `Controller` - декоратор класса, определяющий маршрутный узел и осуществляющий перенос декорированных
  данных в случае наследования маршрутных узлов
...
- `middlewares` - для обозначения middleware-функций, "мостов" и расширения контекста. Список включает
  в себя: `Middleware`, `Use`, `Bridge`, `Marker` и `UseNext`
...
Для маршрутного узла следует использовать декоратор `@Controller`. В общем случае он является опциональным,
и обязательный только в случае, если вы создается маршрутный узел, наследуя его из другого маршрутного узла.
Подробнее об этом в разделе [`Наследование`](./inherits).
