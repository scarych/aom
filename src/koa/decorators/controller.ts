...
export function Controller(): ClassDecorator {
  return (constructor): void => {
    Reflect.defineMetadata(constants.CONTROLLER_NODE, true, constructor);

    // берем родителя, с которого будуте перенесены все значения декораторов и окружения
    function inheritDecorators(parentConstructor) {
      ...
      if (parentEndpoints.length > 0) {
        ...
        const endpoints: IEndpoint[] =
          Reflect.getOwnMetadata(ENDPOINTS_METADATA, constructor) || [];
      ...
      if (!Reflect.getOwnMetadata(constants.CONTROLLER_NODE, parentConstructor)) {
        inheritDecorators(Object.getPrototypeOf(parentConstructor));
      }
    }
    // вызовем операцию переноса декоратором с родителя на текущий класс
    inheritDecorators(Object.getPrototypeOf(constructor));
  };
}
